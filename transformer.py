...
class Transformer(torch.nn.Module):
    ...
    def forward(self, input_data,positional,input_mask):
        emb_ln, emb = self.embedding.forward(input_data,positional) 
        
        z, total_kl = self.sgp_layer_list[0].forward(emb_ln, self.keys[0], input_mask) 
    ...
    def pred_nll(self, data_loader):
        ...
        for i in range(data_loader.num_batches):
            _,input_data,input_mask, positional,answers=data_loader.__load_next__()
            if self.sample_size == 1 and self.flag_sgp: 
                logits = torch.stack([self.forward(input_data,positional,input_mask)[0] for _ in range(5)])
                pred_probs = torch.mean(torch.softmax(logits, -1), 0) 
            else:
                logits, _ = self.forward(input_data,positional,input_mask) 
                logits = logits.reshape(-1, self.sample_size, self.num_class) 
